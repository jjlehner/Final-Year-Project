...
-- | Data type used to represent a signal changing value
...
-- | Data type manipulated by testbenches using EDSL
...
-- | Execute all events in a timeslot, execute a single event may generate more events.
-- This function will only return once these extra events have been executed.
executeTimeSlot ::
    ExpandedIR
...
-- | Given a statement item from a process block, generate a time slot event
...
-- | Apply updates events generated by a process block
updateTimeSlotFromSensitiveProcess :: AlwaysConstructIR -> State TimeSlot ()
updateTimeSlotFromSensitiveProcess alwaysConstruct =
    do
        timeSlotInit <- get
        put $ foldl (\ts si -> addManyToActiveRegion ts (statementItemToTimeSlotEvent si)) timeSlotInit alwaysConstruct._statementItems
        return ()
...
-- | Given a stimulated circuit from the testbench, this function determines which update events to apply.
...
-- | Given a stimulated circuit from the testbench, determines which update events to apply.
...
-- | Get value of a signal from a ConnectionIR
...
-- | Self-determined expressions within System Verilog define their result type.
-- This function determines what this result type is.
...
-- | Evaluate an expression within the System Verilog simulator
evaluateExpression ::
    ExpandedIR
    -> ExpressionType
    -> DynamicCircuitState
...
-- | Given the change of a signal value,
-- find all signals which depend on this value and update them too
propagateUpdate ::
    ExpandedIR
    -> ConnectionIR
    -> State (DynamicCircuitState, TimeSlot) ()
...
-- | Execute Event and Update Circuit State and Timeslot.
-- Executing an event may add many more events to the timeslot state.
-- This circuit pattern matches against the type of event to execute.
executeEvent ::
    ExpandedIR
    -> TimeSlotEvent
    -> State (DynamicCircuitState, TimeSlot) ()
...
-- | Execute all events in the active region set
...
-- | Execute all events in the re-active region set
...
-- | Given the representation of the circuit and a list of circuit stimulations
-- apply all updates to the circuit and return state of the circuit after all updates have been applied.
eval ::
    ExpandedIR
...
-- | Trigger all combinatorial blocks. This is used to find the initial state of the circuit.
